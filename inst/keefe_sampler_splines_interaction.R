...
y <- beta2*x[,2] + beta1*x[,1]*x[,2] + rnorm(n = n)
...
tau_beta_j_post <- matrix(0,nrow = nmcmc, ncol = p_range)
tau <- 1
acceptance_rate <- 0
...
B2 <- bs(x[,2]) %>% as.matrix()
B3 <- bs(x[,3]) %>% as.matrix()
B4 <- multiply_matrices_general(A = B1,B = B2)
list_B <- list(B1 = B1, B2 = B2, B3 = B3, B4 = B4)

betas <- vector("list", 3)
for(ii in 1:4){
  betas[[ii]] <- matrix(0,ncol = NCOL(list_B[[ii]]))
}
...
a_tau_beta_j_prior <- c(0.1,0.1,0.1,0.1)
d_tau_beta_j_prior <- c(0.1,0.1,0.1,0.1)

tau_beta_j <- rep(1,NCOL(x)+1)
...
for(mcmc_iter in 1:nmcmc){
  ...
  if(verb == "grow"){
    if(length(which(!(1:p_range %in% selected_vars)))!=0){
      new_selected_vars <- sort(unique(c(selected_vars,sample(x = which(!(1:p_range %in% selected_vars)),size = 1))))
    } else {
    ...
    if(length((1:p_range)[!((1:p_range) %in% selected_vars)])!=0){
      c_var <- sample((1:p_range)[!((1:p_range) %in% selected_vars)],size = 1)
...
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="3",3,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="4",4,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="1-2",5,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="1-3",6,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="1-4",7,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="2-3",8,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="2-4",9,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="3-4",10,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="1-2-3",11,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="1-2-4",12,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="1-3-4",13,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="2-3-4",14,selected_vars_list_vec)
selected_vars_list_vec <- ifelse(selected_vars_list_vec=="1-2-3-4",15,selected_vars_list_vec)

par(mfrow=c(1,1))
plot(1:1000,as.numeric(selected_vars_list_vec),axes = FALSE,ylab = "Parameters selected",pch=20)
axis(2,at = 1:15,labels = c("1","2","3","4","1-2","1-3","1-4","2-3","2-4","3-4","1-2-3","1-2-4","1-3-4","2-3-4","1-2-3-4"))
